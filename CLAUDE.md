...
- Each prompt is meant to be copy-pasted whole as the **first message** in a new chat (not a system-prompt field) on `chat.genai.army.mil` (see the settings table in `README.md`).
- **Target model is migrating from Claude to Gemini.** These prompts were originally written and tuned against Anthropic Claude models, but Anthropic/Claude is now banned for DoD use, so `chat.genai.army.mil` will run Gemini going forward. Treat any remaining Claude-specific assumptions (tag-following quirks, phrasing tuned to Claude's instruction-following style, the README's settings table still listing a Claude model) as stale and due for review — when editing a prompt, check whether its instructions still hold for Gemini rather than assuming Claude-era behavior carries over, and flag/update the README settings table when you touch model-dependent behavior.
...
- Several rules require **exact, verbatim phrasing** the model must reproduce with no paraphrasing (e.g. `supervisor_prompt.txt`'s three approved concurrence openers, checked in `assessment_review_prompt.txt`'s CHECK 1). When editing these, grep the other prompt files for the same phrases — they cross-reference each other and must stay consistent, or update all references together.
...
`docs/superpowers/specs/2026-07-14-assessment-package-template-design.md` §6 records that `contribution_plan_prompt.txt` and `supervisor_prompt.txt` currently conflict with the 2026 guide's requirements captured in the new template (labeled JA/CT/MS contributions vs. unlabeled narrative objectives; Meeting/Exceeding/Partially-meeting/Not-meeting lead-ins vs. the current concur/partially-concur/do-not-concur openers; abbreviated `(W)/(R)/(I)` labels vs. spelled-out `What:/Result:/Impact:` in `wri_prompt.txt`). Do not "fix" these silently — they are an intentionally deferred follow-up project; check the spec before changing this behavior.
